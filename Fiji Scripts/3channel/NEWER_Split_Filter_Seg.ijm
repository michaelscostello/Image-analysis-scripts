    ...
    Ext.getSizeZ(sizeZ);
  	...
  	run("Bio-Formats Importer", "open=&path autoscale color_mode=Default view=Hyperstack stack_order=XYCZT series_");
		...
		selectWindow("C1-" + image);
			...
			run("3D Filter Objects", "descriptor=Volume(Pix) min=100 max=1000");
			selectWindow("C1-ChannelGreen-488,Red-561,FarRed-640_Seq0000.nd2merge_filtered");
			run("3D Simple Segmentation", "low_threshold=50 min_size=10 max_size=1000");
			close("C1-ChannelGreen-488,Red-561,FarRed-640_Seq0000.nd2merge_filtered");
			...
			//selectWindow("3D_Median");
			run("3D Filter Objects", "descriptor=Volume(Pix) min=100 max=1000");
			selectWindow("C2-ChannelGreen-488,Red-561,FarRed-640_Seq0000.nd2merge_filtered");
			run("3D Simple Segmentation", "low_threshold=50 min_size=10 max_size=1000");
			//saveAs("tiff, output+image+568seg.tif");
			close("C2-ChannelGreen-488,Red-561,FarRed-640_Seq0000.nd2merge_filtered");
			run("3D Manager");
			Ext.Manager3D_AddImage();
			Ext.Manager3D_SelectAll();
			Ext.Manager3D_Measure();
